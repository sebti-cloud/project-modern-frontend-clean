...
const App = () => {
  ...
  const fetchOldProducts = async (category = 'all') => {
    try {
      ...
      const response = await fetch(url);
      const data = await response.json();
      setOldProducts(Array.isArray(data) ? data : []);
    } catch (error) {
  ...
  return (
    <Router>
      ...
      <Routes>
        ...
        <Route path="/top-products" element={<TopProducts topProducts={topProducts} fetchTopProducts={fetchTopProducts} addtocart={addtocart} />} />
        <Route path="/old-product" element={<OldProduct oldProducts={oldProducts} fetchOldProducts={fetchOldProducts} addtocart={addtocart} />} />
        <Route path="/*" element={
