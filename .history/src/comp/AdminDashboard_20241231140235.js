...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    console.log('Token récupéré:', token); // Ajout du log pour le token
    if (token) {
      try {
        const decodedToken = jwtDecode(token);
        console.log('Token décodé:', decodedToken); // Ajout du log pour le token décodé
        setRole(decodedToken.role);
        setIsAuthenticated(true);
      } catch (err) {
        console.error('Invalid token:', err); // Ajout du log pour les erreurs de token
