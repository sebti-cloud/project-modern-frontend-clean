...
const Orders = () => {
  ...
  const handleUpdateOrder = async () => {
    if (selectedOrder) {
      try {
        const response = await fetch(`http://localhost:3001/api/update-tracking/${selectedOrder.id}`, {
          method: 'PUT',
          ...
          <tbody>
            {orders.map(order => (
              <tr key={order.id}>
                ...
                <td>{order.total_price} MAD</td>
                <td>{new Date(order.created_at).toLocaleString()}</td>
                <td>{order.status}</td> {/* Afficher le statut actuel */}
                <td>{order.tracking_number}</td>
                <td>
                  <button className="validate" onClick={() => handleUpdateOrder(order.id)}>Valider</button>
                  <button className="delete" onClick={() => handleDeleteOrder(order.id)}>Supprimer</button>
