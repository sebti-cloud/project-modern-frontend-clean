...
const Orders = () => {
  ...
  const handleUpdateOrder = async (orderId) => {
    ...
    if (status && paymentMethod) {
      try {
        ...
        setPaymentMethods(prevMethods => ({ ...prevMethods, [orderId]: '' }));
      } catch (error) {
  ...
  const handlePaymentMethodChange = (orderId, newPaymentMethod) => {
    setPaymentMethods(prevMethods => ({ ...prevMethods, [orderId]: newPaymentMethod }));
  };
          ...
          <tbody>
            {orders.map(order => (
              <tr key={order.id}>
                ...
                <td>
                  <select value={paymentMethods[order.id] || order.payment_method || ''} onChange={(e) => handlePaymentMethodChange(order.id, e.target.value)}>
...
const Orders = () => {
  ...
  const handleUpdateOrder = async (orderId) => {
    const status = statuses[orderId] || orders.find(order => order.id === orderId)?.status;
    const paymentMethod = paymentMethods[orderId] || orders.find(order => order.id === orderId)?.payment_method;
    console.log('Updating order:', orderId, 'Status:', status, 'Payment Method:', paymentMethod); // Log update data
    if (status && paymentMethod) {
      try {
        const response = await fetch(`http://localhost:3001/api/update-tracking/${orderId}`, {
          method: 'PUT',
          ...
          <tbody>
            {orders.map(order => (
              <tr key={order.id}>
                ...
                <td>
                  <select value={paymentMethods[order.id] || order.payment_method || ''} onChange={(e) => handlePaymentMethodChange(order.id, e.target.value)}>
                    <option value="pending_payment">En attente de paiement</option> {/* Ajout du nouvel état */}
                    <option value="paid">Payé</option>
                    <option value="cash on delivery">Paiement à la livraison</option>
                  </select>
                </td>
