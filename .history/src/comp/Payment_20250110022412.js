...
const Payment = ({ amount }) => {
  ...
  useEffect(() => {
    ...
    if (!clientId) {
      console.error("Le client-id PayPal n'est pas défini. Veuillez vérifier votre fichier .env.");
      return;
    }

    console.log("Client ID PayPal:", clientId);
        ...
        window.paypal.Buttons({
          createOrder: function(data, actions) {
            return actions.order.create({
              purchase_units: [{
                amount: {
                  value: amount.toString(), // Assurez-vous que la valeur est une chaîne de caractères
                },
              }],
            }).catch((err) => {
