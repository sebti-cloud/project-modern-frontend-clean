...
const Payment = () => {
  const { orderId } = useParams(); // Récupérer l'ID de la commande depuis les paramètres de l'URL
  const [total, setTotal] = useState(null); // Utiliser 'total' pour le montant de la commande
  ...
  useEffect(() => {
    const fetchTotal = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/order-total/${orderId}`); // Assurez-vous que cette URL correspond à votre API
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        setTotal(data.total); // Assigner la valeur de 'total' récupérée depuis la base de données
      } catch (error) {
      ...
      window.paypal.Buttons({
        createOrder: function(data, actions) {
          return actions.order.create({
            purchase_units: [{
              amount: {
                value: total.toString(), // Utiliser 'total' pour le montant
              },
            }],
          }).catch((err) => {
