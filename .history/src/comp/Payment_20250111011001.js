...
import './payment.css';
import { useNavigate, useParams, Link } from 'react-router-dom'; // Assurez-vous que Link est importé ici
...
const Payment = () => {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/order-details/${orderId}`);
        if (!response.ok) {
