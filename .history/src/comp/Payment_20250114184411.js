...
const Payment = () => {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      ...
      try {
        ...
        setTrackingNumber(data.trackingNumber || ''); // Assurez-vous que cela correspond au nom exact de la propriété
        setUserName(data.userName || ''); // Assurez-vous que cela correspond au nom exact de la propriété
  
        if (!data.email) {
          console.error("Email non défini.");
        }
        if (!data.trackingNumber) { // Assurez-vous que cela correspond au nom exact de la propriété
  ...
  return (
    <div>
      ...
      <p>Total: {orderDetails.total} Dhs</p>
      <p>Email: {orderDetails.email}</p>
      <p>Numéro de suivi: {orderDetails.trackingNumber}</p>
      <p>Nom du client: {orderDetails.userName}</p>
    </div>
  );
};
