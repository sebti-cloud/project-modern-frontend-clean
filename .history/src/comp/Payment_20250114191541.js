          ...
          return actions.order.capture().then(async function(details) {
            ...
            try {
              const response = await fetch('${process.env.REACT_APP_API_URL}/api/checkout', {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ 
                  cart: [], // Vous devez passer votre panier ici
                  userInfo: { 
                    name: userName.split(' ')[0], 
                    surname: userName.split(' ')[1], 
                    phone: '', // Vous devez passer le numéro de téléphone ici
                    email: userEmail,
                    address: '', // Vous devez passer l'adresse ici
              ...
              console.log('Commande enregistrée avec succès après la confirmation du paiement.');
            } catch (error) {
