...
const Payment = () => {
  ...
  const handleOnlineCheckout = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/online-checkout', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          cart: [], // Remplacez par votre panier
          userInfo: {
            name: userName.split(' ')[0],
            surname: userName.split(' ')[1],
            phone: '', // Remplacez par le téléphone de l'utilisateur
            email: userEmail,
            address: '', // Remplacez par l'adresse de l'utilisateur
            paymentMethod: 'paid', // Indique que le paiement est effectué en ligne
          },
        }),
      });
      ...
      console.log('Commande en ligne enregistrée avec succès.');
    } catch (error) {
      console.error('Erreur lors de l\'enregistrement de la commande en ligne :', error);
    }
  };
      ...
      window.paypal.Buttons({
        ...
        onApprove: function(data, actions) {
          return actions.order.capture().then(async function(details) {
            ...
            // Appeler la fonction pour enregistrer la commande en ligne
  ...
  }, [total, navigate, paypalLoaded, userEmail, userName]);
