...
const Payment = () => {
  const { orderId } = useParams(); // Assurez-vous que orderId est bien récupéré ici
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      ...
      try {
        ...
        setTrackingNumber(data.trackingnumber || ''); // Propriété correcte
        setUserName(data.username || ''); // Propriété correcte

        if (!data.email) {
          console.error("Email non défini.");
        }
        if (!data.trackingnumber) { // Propriété correcte
