...
const Payment = () => {
  ...
  const updatePaymentStatus = async (pendingOrderId, transactionId) => {
    console.log('Mise à jour du statut de paiement et du Transaction ID pour Pending Order ID:', pendingOrderId);
    try {
      ...
      const data = await response.json();
      console.log('Statut de paiement et Transaction ID mis à jour avec succès:', data);
    } catch (error) {
      console.error('Erreur lors de la mise à jour du statut de paiement et du Transaction ID :', error);
    }
  };
