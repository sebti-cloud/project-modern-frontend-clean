...
const Payment = () => {
  ...
  const updatePaymentStatus = async (pendingOrderId, transactionId) => {
    console.log('Mise à jour du Transaction ID et du statut de paiement pour Pending Order ID:', pendingOrderId, 'avec Transaction ID:', transactionId);
    try {
      ...
      const data = await response.json();
      console.log('Transaction ID et statut de paiement mis à jour avec succès:', data);
    } catch (error) {
