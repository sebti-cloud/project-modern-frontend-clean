...
const Products = () => {
  ...
  const handleAddProduct = async (e) => {
    ...
    formData.append('types', newProduct.types);
  ...
  const handleUpdateProduct = async (e) => {
    ...
    formData.append('types', editProduct.types);
  ...
  return (
    <div className="products">
      ...
      <form onSubmit={editProduct ? handleUpdateProduct : handleAddProduct}>
        ...
        <textarea type="text" className="details" placeholder="Details" value={editProduct ? editProduct.details : newProduct.details} onChange={handleChange} required></textarea>
        ...
        <tbody>
          {products.map(product => (
            <tr key={product.id}>
              ...
              <td>{product.types.join(', ')}</td> {/* Affichage des types multiples */}
              ...
              <td>
                ...
                <select value={product.types[0]} onChange={(e) => handleUpdateType(product.id, e.target.value)} className="type-select">
                  <option value="featured">Featured</option>
                  <option value="new">New</option>
                  <option value="top">Top</option>
