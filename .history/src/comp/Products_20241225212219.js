...
const Products = () => {
  ...
  const handleLikeProduct = async (id) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/products/${id}/like`, {
        method: 'POST',
      });
      if (response.ok) {
        fetchProducts();
        alert('Product liked successfully!', 'alert-success');
      } else {
        alert('Failed to like product.', 'alert-danger');
      }
    } catch (error) {
      ...
      <table className="products-table">
        <thead>
          <tr>
            ...
            <th>Price</th>
            <th>Details</th>
            <th>Likes</th> {/* Colonne pour afficher les likes */}
            <th>Actions</th>
          </tr>
        </thead>
        <tbody>
          {products.map(product => (
            <tr key={product.id}>
              ...
              <td>{product.likes}</td> {/* Affichage des likes */}
              <td>
                ...
                <button onClick={() => handleLikeProduct(product.id)} className="like-button">Like</button> {/* Bouton pour liker */}
