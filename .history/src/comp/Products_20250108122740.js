...
const Products = () => {
  ...
  const handleEditProduct = (product) => {
    setEditProduct({ ...product, types: product.types || [] });
  };
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      ...
      // Mettez à jour l'état avec les informations du produit mis à jour
      ...
      console.error('Error updating product:', error);
      alert(`Failed to update product: ${error.message}`, 'alert-danger');
    }
  };
    ...
    <div className="admin-dashboard">
      ...
      <div className="products">
        ...
        <form onSubmit={editProduct ? handleUpdateProduct : handleAddProduct}>
          ...
          <select
            name="types"
            value={editProduct ? (editProduct.types && editProduct.types.length ? editProduct.types : []) : (newProduct.types.length ? newProduct.types : [])}
            onChange={handleChange}
            multiple
            required
          >
