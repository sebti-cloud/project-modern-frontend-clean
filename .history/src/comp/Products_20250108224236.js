...
const Products = () => {
  ...
  const handleUpdateType = async (productId, newType) => {
    try {
        console.log(`Updating product with ID: ${productId}`); // Log pour vérifier l'ID du produit
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/products/${productId}`);
        if (!response.ok) {
            throw new Error('Product not found');
        }
        const productData = await response.json();
        console.log('Current product data:', productData); // Log pour vérifier les données actuelles du produit
        let types = productData.types || [];

        // Si le type sélectionné est déjà dans le tableau, nous le supprimons
        if (types.includes(newType)) {
            types = types.filter(type => type !== newType);
        } else {
            // Sinon, nous l'ajoutons
            types.push(newType);
        }

        console.log('Updated types:', types); // Log pour vérifier les types mis à jour

        const responseUpdate = await fetch(`${process.env.REACT_APP_API_URL}/api/products/${productId}/type`, {
            method: 'PUT',
        ...
        alert(`Error updating product type: ${error.message}`, 'alert-danger');
    }
};
            ...
            {products.map(product => (
              <tr key={product.id}>
                ...
                <td>
                  ...
                  <select value={product.types && product.types[0] ? product.types[0] : ''} onChange={(e) => handleUpdateType(product.id, e.target.value)} className="type-select">
                    <option value="featured">Featured</option>
                    <option value="new">New</option>
                    <option value="top">Top</option>
                    <option value="sale">Sale</option>
...
const Products = () => {
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to update product: ${responseData.message}`, 'alert-danger');
      }
    } catch (error) {
      console.error('Error updating product:', error);
    }
  ...
  const handleProductUpdate = async (product) => {
    try {
      ...
      // Mettez à jour l'état ou faites quelque chose avec les données mises à jour
    } catch (error) {
      console.error('Error updating product:', error);
    }
  };
    ...
    <div className="admin-dashboard">
      ...
      <div className="products">
        ...
        <form onSubmit={editProduct ? handleUpdateProduct : handleAddProduct}>
          ...
          <select
            name="types"
            value={editProduct ? editProduct.types : newProduct.types} // Assurez-vous que value est un tableau
            onChange={handleChange}
            multiple
            required
          >
