...
const UserActivity = () => {
  ...
  const fetchPurchases = async () => {
    try {
      ...
      setPurchases(data);
      generateChartData(data); // Générer le graphique basé sur les données réelles
    } catch (error) {
  ...
  const generateChartData = (purchases) => {
    const categoryCount = {};
    ...
    const chartData = Object.keys(categoryCount).map((category) => ({
      ...
      percentage: ((categoryCount[category] / totalPurchases) * 100).toFixed(2),
