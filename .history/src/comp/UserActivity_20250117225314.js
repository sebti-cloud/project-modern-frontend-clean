...
const UserActivity = () => {
  ...
  const fetchPurchases = async () => {
    try {
      ...
      setPurchases(data);
      fetchProductCategories(data); // Récupérer les catégories des produits achetés
    } catch (error) {
  ...
  const fetchProductCategories = async (purchases) => {
    try {
      ...
      const data = await response.json();
      generateChartData(data); // Générer le graphique basé sur les catégories des produits
    } catch (error) {
