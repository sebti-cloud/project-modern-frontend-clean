...
const UserActivity = () => {
  ...
  const fetchProductCategories = async (purchases) => {
    try {
      ...
      const data = await response.json();
      console.log('Product Categories:', data); // Ajouter un log pour vérifier les catégories des produits
  ...
  const generateChartData = (products) => {
    ...
    console.log('Chart Data:', chartData); // Ajouter un log pour vérifier les données du graphique
