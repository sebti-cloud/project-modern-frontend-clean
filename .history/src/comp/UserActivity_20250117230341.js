...
const UserActivity = () => {
  ...
  const fetchProductCategories = async (purchases) => {
    try {
      console.log('Purchases:', purchases); // Ajouter un log pour vérifier la structure des données d'achats
      const productIds = purchases.map(purchase => purchase.product_id);
      console.log('Product IDs:', productIds); // Vérifier les IDs des produits
      ...
      const data = await response.json();
      console.log('Product Categories:', data); // Vérifier les catégories des produits
  ...
  const generateChartData = (products) => {
    ...
    console.log('Chart Data:', chartData); // Vérifier les données du graphique
