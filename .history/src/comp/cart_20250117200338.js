...
const Cart = ({ cart, setCart }) => {
  ...
  const handleCheckout = async (e) => {
    ...
    if (response.ok) {
      ...
      console.log('orderId:', data.orderId); // Ajoutez un log pour vérifier la valeur de orderId
    ...
    if (response.ok) {
      ...
      for (const item of cart) {
        await fetch('${process.env.REACT_APP_API_URL}/api/purchases', {
          ...
          body: JSON.stringify({ userId: data.userId, product: item.name }) // Utilisation du champ correct pour userId
        });
      }
