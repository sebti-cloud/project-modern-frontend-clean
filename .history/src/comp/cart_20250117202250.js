  ...
  const handleCheckout = async (e) => {
    ...
    if (response.ok) {
      ...
      for (const item of cart) {
        await fetch('${process.env.REACT_APP_API_URL}/api/purchases', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
          body: JSON.stringify({ userId: data.userId, product: item.name })
        });
      }
...
const Cart = ({ cart, setCart }) => {
  const [checkout, setCheckout] = useState(false);
  const [userInfo, setUserInfo] = useState({ name: '', surname: '', phone: '', email: '', address: '', paymentMethod: '', id: 1 });
  const [trackingNumber, setTrackingNumber] = useState('');
  const [modalIsOpen, setModalIsOpen] = useState(false);
  const [cartCount, setCartCount] = useState(parseInt(localStorage.getItem('cartCount')) || 0);
  const navigate = useNavigate();
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText(trackingNumber).then(() => {
      ...
      console.error('Erreur lors de la copie du numéro de suivi:', err);
    });
  };
