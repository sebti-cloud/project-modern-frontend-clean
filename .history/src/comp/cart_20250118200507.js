...
const Cart = ({ cart, setCart }) => {
  const [checkout, setCheckout] = useState(false);
  const [userInfo, setUserInfo] = useState({ id: null, name: '', surname: '', phone: '', email: '', address: '', paymentMethod: '' }); // Assurez-vous que id est initialisé à null
  const [trackingNumber, setTrackingNumber] = useState('');
  const [modalIsOpen, setModalIsOpen] = useState(false);
  const [cartCount, setCartCount] = useState(parseInt(localStorage.getItem('cartCount')) || 0); // Initialiser à partir de localStorage
  const navigate = useNavigate();
  ...
  const handleCheckout = async (e) => {
    ...
    // Vérifiez les données userInfo avant de passer la commande
