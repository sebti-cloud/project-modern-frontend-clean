...
const Nav = ({ search, setSearch, searchproduct, setSearchResults, isAuthenticated, handleLogout }) => {
  ...
  const logout = () => {
    ...
    localStorage.removeItem('token'); // Supprimez également le token de localStorage
    handleLogout();
    navigate('/login');
  };
      ...
      <div className='last_header'>
        ...
        <div className='nav'>
          <ul>
            ...
            {isAuthenticated && <li><Link to='/profile' className='link'>Mon Compte</Link></li>} {/* Lien vers le compte utilisateur */}
          </ul>
        </div>
        <div className='offer'>
